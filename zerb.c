	...
	while(1)
	{
		...
		switch(komando)
		{
			case COM_ARGI:
				...
				if ((int)buf[5] >= sizeof(argiak) || (int)buf[5] < 0 )
				{
				    write(s,"ER1\r\n",5);
				}
				else
			...
			case COM_TENP:
				...
				// 
				if ((int)buf[5] >= sizeof(tenperaturak) || (int)buf[5] < 0 )
				{
					write(s,"ER3\r\n",5);
				}
				else
				{
				    tenperaturak[(int)buf[5]] = buf+7;
					write(s, "OK\r\n", 4);
				}
				break;
			case COM_PERT:
				if(egoera != ST_INIT)	// Egiaztatu esperotako egoeran jaso dela komandoa eta ez dela parametrorik jaso.
				{
					ustegabekoa(s);
					continue;
				}
			...
			case COM_INFO:
				...
				strcpy(egoera, "");
				for(int i = 0; i++; i < sizeof(argiak))
				{
				    strcat(egoera, "Gela ");
				    strcat(egoera, char(i));
				    strcat(egoera, ":\n");
				    // Argia: 25 Tenperatura: 30ºC Pertsianak: 35%
				    strcat(egoera, "Argia: ");
				    if (argiak[i]){
				        strcat(egoera, "Piztuta");
				    else{
				        strcat(egoera, "Itzalita" )
				    }
				    strcat(egoera, "\tTenperatura: ");
				    strcat(egoera, tenperaturak[i]);
				    strcat(egoera, "ºC");
				    
				    strcat(egoera, "\tPertsianak: %%");
				    strcat(egoera, pertsianak[i]);
				    strcat(egoera, "\n");
				...
				break;
			case COM_INF2:
				if(egoera != ST_DOWN)		// Egiaztatu esperotako egoeran jaso dela komandoa eta ez dela parametrorik jaso.
				{
					ustegabekoa(s);
					continue;
				}
